    ...
    def project(self, full_rank_grad, iter):
        ...
        if self.proj_type == 'std':
            if full_rank_grad.shape[0] >= full_rank_grad.shape[1]:
                ...
                self.d_cur = low_rank_grad / torch.linalg.matrix_norm(
                    low_rank_grad,
                    ord="fro"
                ).clamp_min(1e-8)
        ...
        self.adaptive_projection_changing = (self.current_iteration != 1) and (self.current_iteration % self.update_proj_gap == 0) and (
                (torch.linalg.matrix_norm((self.d_cur - self.d_init),
                                          ord='fro') / self.current_iteration).item() < self.gamma)
