...
from adarankgrad_projector import *
import time
...
def add_optimizer_params(parser: argparse.ArgumentParser):
    ...
    #adarankgrad parameter
    parser.add_argument("--no_deprecation_warning", type=bool, default=False)
    parser.add_argument("--grad_matrix_norm_thresh", type=float, default=0.001)



class AdamW_adarankgrad(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.98), eps=1e-6, weight_decay=0.0, correct_bias=True, galore_rank=4,
                 ...
                 no_deprecation_warning = False, grad_matrix_norm_thresh = 0.001):
        if lr < 0.0:
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, correct_bias=correct_bias,
                        ...
                        no_deprecation_warning=no_deprecation_warning, grad_matrix_norm_thresh=grad_matrix_norm_thresh)
    ...
    def step(self, closure: Callable = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if "step" not in state:
                    state["step"] = 0
                # Low Rank Projection
                if "max_rank" in group:
                    if "projector" not in state:
                        state["projector"] = adarankgradProjector(
                            min_rank=group["min_rank"],
                            max_rank=group["max_rank"],
                            rank_step=1 if 'rank_step' not in group else group['rank_step'],
                            update_proj_gap=group["update_proj_gap"],
                            scale=group["scale"],
                            proj_type=group["proj_type"],
                            xi_threshold=group["xi_threshold"]
                        )
                    if "low_gradients" not in state:
                        if "n" not in state:
                            n, m = grad.shape
                            state["n"] = n
                            state["m"] = m
                        grad, ada_rank, flag_updated = state["projector"].project(
                            grad,
                            state["step"],
                            update_projection_matrix=True)
                    elif state["low_gradients"]:
                        # print("Updated proj matrix")
                        grad, ada_rank, flag_updated = state["projector"].project(
                            grad,
                            state["step"],
                            update_projection_matrix=True)
                        ...
                        grad, ada_rank, flag_updated = state["projector"].project(grad, state["step"])
                    ...
                    if flag_updated:
                        ...
                        if "exp_avg" not in state:
                            ...
                            n, m = grad.shape
                            n_, m_ = state["exp_avg"].shape
                            if ada_rank == m: # right projection
                                if ada_rank <= m_:
                                  state["exp_avg"] = state["exp_avg"][:, :ada_rank]
                                  state["exp_avg_sq"] = state["exp_avg_sq"][:, :ada_rank]
                                else:
                                    temp = torch.zeros((n_, ada_rank), device=state["exp_avg"].device, dtype=state["exp_avg"].dtype)
                                    temp[:, :m_] = state["exp_avg"]
                                    state["exp_avg"] = temp
                                    temp = torch.zeros((n_, ada_rank), device=state["exp_avg_sq"].device, dtype=state["exp_avg_sq"].dtype)
                                    temp[:, :m_] = state["exp_avg_sq"]
                                    state["exp_avg_sq"] = temp
                            if ada_rank == n: # left projection
                                if ada_rank <= n_:
                                    state["exp_avg"] = state["exp_avg"][:ada_rank, :]
                                    state["exp_avg_sq"] = state["exp_avg_sq"][:ada_rank, :]
                                else:
                                    temp = torch.zeros((ada_rank, m_), device=state["exp_avg"].device, dtype=state["exp_avg"].dtype)
                                    temp[:n_, :] = state["exp_avg"]
                                    state["exp_avg"] = temp
                                    temp = torch.zeros((ada_rank, m_), device=state["exp_avg_sq"].device, dtype=state["exp_avg_sq"].dtype)
                                    temp[:n_, :] = state["exp_avg_sq"]
                                    state["exp_avg_sq"] = temp


                    grad_norm = torch.norm(grad)
                    mean_grad_norm.append(grad_norm)
                    if grad_norm < group["grad_matrix_norm_thresh"]:
                        state["low_gradients"] = True
                    else:
                        state["low_gradients"] = False
                    if "is_attention" in group.keys():
                        if group["is_attention"]:
                            attention_ranks.append((ada_rank, p.grad.size(0), p.grad.size(1)))
                        else:
                            mlp_ranks.append((ada_rank, p.grad.size(0), p.grad.size(1)))
                ...
                # Decay the first and second moment runrning average coefficient
        ...
        self.log_dict["mlp_ranks"] = mlp_ranks
        self.log_dict["attention_ranks"] = attention_ranks

        self.log_dict["low_rank_updates"] = low_rank_updates
        self.log_dict["mean_grad_norm"] = mean_grad_norm
