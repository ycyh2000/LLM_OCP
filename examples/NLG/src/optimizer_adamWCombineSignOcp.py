    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                momentum_buffer.mul_(beta1).add_(grad, alpha=1.0 - beta1)
                velocity_buffer.mul_(beta2).addcmul_(grad, grad, value=1.0 - beta2)
                denom = velocity_buffer.sqrt().add_(group["eps"])
                ...
                p.data.addcdiv_(-step_size, momentum_buffer, denom)
                ...
                Phi = lr * torch.sign(momentum_buffer)   # Initial value
                max_iterations = min(state['step'], 2)

                for _ in range(max_iterations):
                    # Use the exact same formula as MyOptimizer
                    Phi = lr * torch.sign(momentum_buffer) + (1 - lr * velocity_buffer_correct) * Phi.detach()
