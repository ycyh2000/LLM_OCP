...
def add_optimizer_params(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--inner_max', default=2, type=int, help='maximum iterations for inner loop in ocp')
...
class AdamW(Optimizer):
    ...
    def __init__(self, params, lr=1e-4, betas=(0.9, 0.999), eps=1e-6, weight_decay=0.0, correct_bias=True, galore_rank=4, update_proj_gap=200, scale=1.0, proj_type='std',
                 inner_max=100):
        if lr < 0.0:
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, correct_bias=correct_bias, galore_rank=galore_rank, update_proj_gap=update_proj_gap, scale=scale, proj_type=proj_type,
                        inner_max=inner_max)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if 'velocity_buffer' not in state:
                    state['velocity_buffer'] = torch.zeros_like(grad)
                ...
                max_iterations = min(state['step'], inner_max)
                ...
                p.data.add_(-norm_grad)  # Note: here is -Phi
