...
class ocpGNCorrect_CLEARWithoutApproximation(Optimizer):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if "galore_rank" in group:
                    if "projector_grad" not in state:
                        if group['dim'] <= 2:
                            state["projector_grad"] = CLEARProjector(group["galore_rank"],
                                                                update_proj_gap=group["update_proj_gap"],
                                                                scale=group["scale"],
                                                                proj_type=group["proj_type"],
                                                                gamma=group["gamma"]
                                                                )
                            state["projector_grad_grad"] = CLEARProjector(group["galore_rank"],
                                                                update_proj_gap=group["update_proj_gap"],
                                                                scale=group["scale"],
                                                                proj_type=group["proj_type"],
                                                                gamma=group["gamma"]
                            ...
                            state["projector_grad"] = CLEARProjectorTensor(group["galore_rank"],
                                                                      update_proj_gap=group["update_proj_gap"],
                                                                      scale=group["scale"],
                                                                      proj_type=group["proj_type"],
                                                                      gamma=group["gamma"]
                                                                      )
                            state["projector_grad_grad"] = CLEARProjectorTensor(group["galore_rank"],
                                                                      update_proj_gap=group["update_proj_gap"],
                                                                      scale=group["scale"],
                                                                      proj_type=group["proj_type"],
                                                                      gamma=group["gamma"]
                    ...
                    # grad = state["projector_grad"].project(grad, state["step"])

                    # start = time.perf_counter()
                    if grad.dim() >= 2:
                        grad_lowRank = state["projector_grad"].project(grad, state["step"])
                ...
                if "exp_avg" not in state:
                    ...
                    state["exp_avg"] = torch.zeros_like(grad_lowRank)
                    # Exponential moving average of squared gradient values
                    state["exp_avg_sq"] = torch.zeros_like(grad_lowRank)
                ...
                exp_avg.mul_(beta1).add_(grad_lowRank, alpha=(1.0 - beta1))

                grad_sq_lowRank = state["projector_grad_grad"].project(
                    grad.square(),
                    state["step"]
                )
                ...
                exp_avg_full = state["projector_grad"].project_back(exp_avg)

                exp_avg_sq_full = state["projector_grad_grad"].project_back(
                    exp_avg_sq
                )
                exp_avg_sq_full = exp_avg_sq_full.clamp_min(0.0)

                max_iterations = min(state["step"], 1)

                scaled_second_moment = torch.clamp(
                    lr * exp_avg_sq_full,
                    min=0.2,
                    max=0.8
                )
                ...
                Phi = exp_avg_full * (
                        1.0 - (1.0 - scaled_second_moment) ** max_iterations
                ) / (
                              lr * exp_avg_sq_full + 1e-8
                      )

                Phi = torch.clamp(Phi, min=-1.0, max=1.0)
                Phi = lr * Phi

                p.add_(Phi, alpha=-1.0)

                # Phi = exp_avg_full * (
                #         1 - (1 - torch.clamp(lr * exp_avg_sq_full, min=0.2, max=0.8)) ** max_iterations) / (
                #               lr * exp_avg_sq_full + 1e-8)
                ...
                # if "galore_rank" in group:
                #     # norm_grad = state["projector_grad"].project_back(norm_grad)
                #
                #     if grad_lowRank.dim() >= 2:
                #         Phi = state["projector_grad"].project_back(Phi)
                # p.add_(Phi, alpha=-1)
