...
from lotus_projector import lotusProjector
from lotus_projector_tensor import lotusProjectorTensor
import time
...
class ocpGNCorrect_lotus(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.98), eps=1e-6, weight_decay=0.0, correct_bias=True, galore_rank=4,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, correct_bias=correct_bias,
                        galore_rank=galore_rank, update_proj_gap=update_proj_gap, scale=scale, proj_type=proj_type, gamma=gamma)
            ...
            for p in group["params"]:
                ...
                if "galore_rank" in group:
                    if "projector" not in state:
                        if group['dim'] <= 2:
                            state["projector"] = lotusProjector(group["galore_rank"],
                            ...
                            state["projector"] = lotusProjectorTensor(group["galore_rank"],
                                                                       update_proj_gap=group["update_proj_gap"],
                                                                       scale=group["scale"],
                                                                       proj_type=group["proj_type"],
                                                                       gamma=group["gamma"]
