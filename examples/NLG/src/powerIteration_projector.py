...
class PowerIterationSVDProjector:
    def __init__(
        self,
        rank,
        verbose=False,
    ...
    def get_orthogonal_matrix(self, weights, rank, type):
        ...
        U, S, Vh = self._power_iteration_svd(matrix, rank)
    ...
    @torch.no_grad()
    def _power_iteration_svd(self, matrix, rank):
        """
        Basic Power Iteration SVD approximation.

        It approximates:
            matrix ≈ U S Vh

        No randomized SVD small-matrix decomposition is used here.
        """
        m, n = matrix.shape

        # Initialize right subspace Q: [n, rank]
        Q = torch.randn(n, rank, device=matrix.device, dtype=matrix.dtype)
        ...
        # Estimate singular values and right vectors through small projection
        B = U.t() @ matrix              # [rank, n]
