...
def sparsify_topk(tensor, sparsity):
    """
    sparsity = 0.9 means keeping only 10% largest elements.
    """
    if sparsity <= 0:
        return tensor

    numel = tensor.numel()
    k = max(1, int(numel * (1 - sparsity)))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                p.data.add_(-sparsify_topk(Phi, 0.9))  # Note: here is -Phi
